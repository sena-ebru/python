...
def create_report(data):
    ...
    for timestamp, camera_data in data.items():
        for uuid, counts in camera_data.items():
            ...
            plt.figure(figsize=(6, 3))  # Boyutları küçülttüm
            ...
            # Oluşturulan grafik dosyasını bellekte saklamak
            buffer = BytesIO()
            plt.savefig(buffer, format='png')
            plt.close()

            # Grafik dosyasını rapora eklemek
            img = Image(buffer)
            elements.append(img)
